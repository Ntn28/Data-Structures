...
struct dic_table *set_table_new(const size_t initial_size) 
{
...
int resize_and_rehash(struct dic_table *table)
{
...
int dic_search(struct dic_table *table, const char *key, const size_t key_len)
{
...
int dic_remove(struct dic_table *table, const char *key, const size_t key_len) 
{
    size_t hash = djb33x_hash(key, key_len);
    size_t index = hash % table->hashmap_size;
...
void print_dic(struct dic_table *table)
{
    for (size_t i = 0; i < table->hashmap_size; i++) 
    {
        struct dic_node *current = table->nodes[i];
